...
function insertCardToIndex(cardHtml, indexContent) {
  ...
  if (!indexContent.includes(marker)) {
    console.warn("경고: SCHEDULED_POSTS_START 마커를 찾을 수 없습니다.");
    return indexContent;
  }
...
function addUrlToSitemap(filename, publishDate, sitemapContent) {
  const newUrl = `  <url>
    <loc>https://blog5.starrope2023.com/posts/${filename}</loc>
...
function main() {
  ...
  for (const post of schedule.posts) {
    ...
    console.log(`발행 중: ${post.filename} (${post.publish_date} ${post.publish_time})`);
